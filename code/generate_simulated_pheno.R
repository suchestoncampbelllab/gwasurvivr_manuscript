...
n_generate <- function(n){
    data.frame(
        ...
        write.table(
            paste0("/data/input/impute/sample_files/n", n, ".chr18.impute_sample"),
            sep = " ",
            row.names = FALSE,
            quote = FALSE
        )
...
#### generate sample_ids ####
### sample id dir
myDir <- "/data/input/impute/sample_ids/"
...
#             "/data/input/impute/covariates/simulated_pheno.txt",
#             sep = " ",
#             row.names = FALSE,
#             quote = FALSE)
# write_rds(sim_pheno,
#           "/data/input/impute/covariates/simulated_pheno.rds")

### generate phenotype files for genipe and survivr ####
sim_pheno <- read_rds("/data/input/impute/covariates/simulated_pheno.rds")
for(n in c(100, 1000, 5000)){
    sim_pheno %>%
        slice(1:n) %>%
        select(-ID_1) %>%
        mutate(sex=sex+1) %>%
        write.table(paste0("/data/input/impute/covariates/n", n, "_cov.phenotype"),
